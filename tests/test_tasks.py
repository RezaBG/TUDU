...
async def test_create_task ( setup_task ):
    ...
    logger.info("Create task response: %s", setup_task)
...
async def test_filter_tasks_by_user ( client, setup_user ):
    ...
    await client.post(
        "/tasks", json = generate_task_payload(
            title = "Task 1", description = "Description 1", owner_id = owner_id, status = TaskStatus.PENDING
            )
        )
    await client.post(
        "/tasks", json = generate_task_payload(
            title = "Task 2", description = "Description 2", owner_id = owner_id, status = TaskStatus.IN_PROGRESS
            )
        )
    await client.post(
        "/tasks", json = generate_task_payload(
            title = "Task 3", description = "Description 3", owner_id = owner_id, status = TaskStatus.COMPLETED
            )
        )
